...
CHARACTER(LEN=256)              :: CVAR(4)     !! netCDF variable name
INTEGER(KIND=JPIM)              :: NCID        !! netCDF file     ID
INTEGER(KIND=JPIM)              :: NVARID(4)   !! netCDF variable ID
...
USE YOS_CMF_INPUT,           ONLY: LROSPLIT,  LWEVAP, LWINFILT,   DTIN
...
USE CMF_UTILS_MOD,           ONLY: vecD2mapD,vecP2mapP
#ifdef UseMPI_CMF
...
REAL(KIND=JPRB), INTENT(OUT)     :: PBUFF(:,:,:)
!============================
CALL vecD2mapD(real(D2FLDFRC,kind=JPRB),D2MAPTMP)             !! MPI node data is gathered by vecP2mapR
#ifdef UseMPI_CMF
