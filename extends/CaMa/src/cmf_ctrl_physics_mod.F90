...
USE YOS_CMF_INPUT,         ONLY: LKINE,  LSLPMIX, LFLDOUT,   LPTHOUT,   LDAMOUT, LLEVEE, LOUTINS, LSPAMAT,LSEDIMENT
USE YOS_CMF_PROG,          ONLY: D2FLDOUT, D2FLDOUT_PRE
USE YOS_CMF_INPUT ,        ONLY: LDAMIRR
...
USE CMF_CTRL_DAMOUT_MOD,       ONLY: CMF_DAMOUT_CALC_IRR, CMF_DAMOUT_WATBAL_IRR, CMF_DAMOUT_WRTE_IRR
USE CMF_CTRL_DAMOUT_MOD,       ONLY: CMF_DAMOUT_CALC_ORG, CMF_DAMOUT_WATBAL_ORG, CMF_DAMOUT_WRTE_ORG
USE CMF_CTRL_LEVEE_MOD,        ONLY: CMF_LEVEE_OPT_PTHOUT
USE CMF_CTRL_SED_MOD,      ONLY:  CMF_SED_DIAG_AVEMAX_ADPSTP
...
! --- v4.12: damout before pthout for water buget error
  IF ( LDAMOUT ) THEN
    IF (LDAMIRR) THEN
      CALL CMF_DAMOUT_CALC_IRR            !! reservoir operation
    ELSE
      CALL CMF_DAMOUT_CALC_ORG            !! reservoir operation
    ENDIF
  ENDIF
...
! --- Water budget adjustment and calculate inflow
  ...
  IF ( LDAMOUT ) THEN
    IF (LDAMIRR) THEN
      CALL CMF_DAMOUT_WATBAL_IRR            !! reservoir operation
    ELSE
      CALL CMF_DAMOUT_WATBAL_ORG            !! reservoir operation
    ENDIF
  ENDIF
...
!=== 5. calculate averages, maximum
  ...
  IF( LSEDIMENT )THEN
    CALL CMF_SED_DIAG_AVEMAX_ADPSTP
  ENDIF
!=== option for ILS coupling
