...
module bgc_soil_SoilBiogeochemNLeachingMod

use precision
use MOD_TimeInvariants, only: sf, sf_no3
use MOD_TimeVariables, only: wliq_soisno, sminn_vr, smin_no3_vr
use MOD_1D_Fluxes, only: &
    rnof, rsur, sminn_leached_vr, smin_no3_leached_vr, smin_no3_runoff_vr
...
real(r8),intent(in) :: zi_soi(0:nl_soil)
...
!                disn_conc = 0._r8
!                if (tot_water > 0._r8) then
!                   disn_conc = (sf_no3 * smin_no3_vr(j,i) )/tot_water
!                end if
!
!                ! calculate the N leaching flux as a function of the dissolved
!                ! concentration and the sub-surface drainage flux
!                smin_no3_leached_vr(j,i) = disn_conc * drain_tot
!             else
                ! calculate the dissolved mineral N concentration (gN/kg water)
                ! assumes that 10% of mineral nitrogen is soluble
                disn_conc = 0._r8
...
end module bgc_soil_SoilBiogeochemNLeachingMod
