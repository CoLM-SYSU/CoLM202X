...
!-----------------------------------------------------------------------------
! DESCRIPTION:
!    Data assimilation of brightness temperature from SMAP satellite
!
! AUTHOR:
...
!#############################################################################
      ...
      ELSE
         ...
         file_smap = trim(DEF_DA_obsdir)//'/pre/'//'/SMAP_L1C_TB_D_'// &
...
!#############################################################################
      ...
      IF (has_obs) THEN
         ...
         IF (p_is_io) THEN
            CALL allocate_block_data(grid_smap, pred_tb_h_wgrid_ens, DEF_DA_ENS)
            CALL allocate_block_data(grid_smap, pred_tb_v_wgrid_ens, DEF_DA_ENS)
         ENDIF
         ...
         IF (p_is_worker) THEN
            ...
            ENDIF
         ENDIF
         IF (p_is_io) CALL allocate_block_data(grid_smap, area_wgrid)
         CALL mg2p_smap%pset2grid(area_pset, area_wgrid, msk=filter)
         deallocate (filter)
...
!#############################################################################
      ...
      IF (has_obs) THEN
         ...
         IF (p_is_worker) THEN
            ...
            num_obs_p = count(smap_lat(:) < lat_p_n .and. smap_lat(:) > lat_p_s .and. &
               smap_lon(:) > lon_p_w .and. smap_lon(:) < lon_p_e .and. &
               smap_time(:) - idate_b >= 0 .and. smap_time(:) - idate_e <= 0)
            ...
            IF (num_obs_p > 0) THEN
               ...
               allocate (smap_lat_p     (num_obs_p         ))
               allocate (smap_lon_p     (num_obs_p         ))
               allocate (smap_tb_h_p    (num_obs_p         ))
               allocate (smap_tb_v_p    (num_obs_p         ))
               ...
               index_p = (smap_lat(:) < lat_p_n .and. smap_lat(:) > lat_p_s .and. &
                  smap_lon(:) > lon_p_w .and. smap_lon(:) < lon_p_e .and. &
                  smap_time(:) - idate_b >= 0 .and. smap_time(:) - idate_e <= 0)

               ! crop observations around target patch
               smap_lat_p  = pack(smap_lat , index_p)
               smap_lon_p  = pack(smap_lon , index_p)
               smap_tb_h_p = pack(smap_tb_h, index_p)
               smap_tb_v_p = pack(smap_tb_v, index_p)
               ...
               d_p = 2*6.3781e3*asin(sqrt(sin((smap_lat_p*pi/180 - patchlatr(1))/2.0)**2 + &
                  cos(smap_lat_p*pi/180)*cos(patchlatr(1))*sin((smap_lon_p*pi/180 - patchlonr(1))/2.0)**2))
               ...
               CALL letkf(DEF_DA_ENS, num_obs_p, &
                  pred_tb_h_p_ens, smap_tb_h_p, obs_err, d_p, loc_r, infl, &
                  trans)
         ...
         IF (p_is_worker) THEN
            DO np = 1, numpatch
               ...
               IF (num_obs_p > 0) THEN
                  ...
                  ! calculate distance between observations and target patch
                  d_p = 2*6.3781e3*asin(sqrt(sin((smap_lat_p*pi/180 - patchlatr(np))/2.0)**2 + &
