...
!END plant hydraulic parameters
   ...
   real(r8), allocatable :: a_dzlak        (:,:)  !Lake layer thickness [m]
   real(r8), allocatable :: a_ziarea       (:,:)  !Lake layerinterface area [m2], only for Simstrat
   real(r8), allocatable :: a_z0h            (:)  !Roughness length for sensible heat  [m]
   real(r8), allocatable :: a_z0q            (:)  !Roughness length for latent heat [m]
...
!End Plant Hydraulic parameters
            ...
            allocate (a_dzlak       (nl_lake,numpatch))
            allocate (a_ziarea    (nl_lake+1,numpatch)) 
            allocate (a_z0h                 (numpatch)) 
            allocate (a_z0q                 (numpatch)) 
   ...
   real(r8) um,thvstar,beta,zii,wc,wc2
      ...
      IF (p_is_worker) THEN
         IF (numpatch > 0) THEN
            ...
            CALL acc2d (dzlak       , a_dzlak        )  
            CALL acc2d (ziarea      , a_ziarea       )   
            CALL acc1d (z0h         , a_z0h          )   
            CALL acc1d (z0q         , a_z0q          )   
            CALL acc1d (felak       , a_felak        )   
            CALL acc1d (gamma       , a_gamma        )   
            CALL acc1d (etal        , a_etal         )   
            CALL acc1d (btpri       , a_btpri        )   
            CALL acc1d (frlak       , a_frlak        )   
            CALL acc1d (tmsno       , a_tmsno        )   
            CALL acc1d (tmice       , a_tmice        )   
            CALL acc1d (tmmnw       , a_tmmnw        )   
            CALL acc1d (tmwml       , a_tmwml        )   
            CALL acc1d (tmbot       , a_tmbot        )   
            CALL acc1d (tmups       , a_tmups        )   
            CALL acc1d (mldp        , a_mldp         )   
            CALL acc1d (upsdp       , a_upsdp        )   
            CALL acc1d (icedp       , a_icedp        )   
            CALL acc1d (bicedp      , a_bicedp       )   
            CALL acc1d (wicedp      , a_wicedp       )
            CALL acc1d (CTfrac      , a_CTfrac       )
            CALL acc1d (rhosnw      , a_rhosnw       )   
            CALL acc2d (uwatv       , a_uwatv        )   
            CALL acc2d (vwatv       , a_vwatv        )   
            CALL acc2d (lksal       , a_lksal        )   
            CALL acc2d (tke         , a_tke          )   
            CALL acc1d (etke        , a_etke         )   
            CALL acc2d (eps         , a_eps          )   
            CALL acc2d (num         , a_num          )   
            CALL acc2d (nuh         , a_nuh          )   
            CALL acc2d (lkrho       , a_lkrho        )   
#endif
