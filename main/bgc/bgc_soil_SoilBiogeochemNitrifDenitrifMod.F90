...
#ifdef BGC
#ifdef NITRIF
module bgc_soil_SoilBiogeochemNitrifDenitrifMod
...
!                                          ! soil is assumed to act like peat
     ...
     do j = 1, nl_soil
        ...
        r_min(j) = 2 * surface_tension_water / (rho_w * 9.80616_r8 * abs(smp(j,i)))
...
!        if ( t_soisno(j,i) <= tfrz .and. no_frozen_nitrif_denitrif) then
...
!        endif
        ...
        ! limit to non-frozen soil layers
!        if ( t_soisno(j,i) <= SHR_CONST_TKFRZ .and. no_frozen_nitrif_denitrif ) then
...
end module bgc_soil_SoilBiogeochemNitrifDenitrifMod
#endif
#endif
