...
subroutine CNGResp(i, ps, pe, npcropmin)
  ...
  do m = ps, pe
     ...
     if (ivt >= npcropmin) then ! skip 2 generic crops
        ...
        cpool_livestem_storage_gr_p(m) = cpool_to_livestemc_storage_p(m) * grperc(ivt) * grpnow(ivt) * &
             respfact_livestem_storage
 
        transfer_livestem_gr_p(m) = livestemc_xfer_to_livestemc_p(m) * grperc(ivt) * (1._r8 - grpnow(ivt)) * &
             respfact_livestem_storage
    ...
    ! leaf and fine root growth respiration
     ...
     if (woody(ivt) == 1._r8) then
        ...
        cpool_livecroot_storage_gr_p(m) = cpool_to_livecrootc_storage_p(m) * grperc(ivt) * grpnow(ivt) * &
                                  respfact_livecroot_storage

        transfer_livecroot_gr_p(m) = livecrootc_xfer_to_livecrootc_p(m) * grperc(ivt) * (1._r8 - grpnow(ivt)) * &
                                  respfact_livecroot_storage
