...
CONTAINS
   ...
   SUBROUTINE allocate_Urban_2DFluxes(grid)
      ...
      IF (p_is_io) THEN
         call allocate_block_data (grid, f_t_room   ) !temperature of inner building [K]
         call allocate_block_data (grid, f_tafu     ) !temperature of outer building [K]
         call allocate_block_data (grid, f_fhac     ) !sensible flux from heat or cool AC [W/m2]
         call allocate_block_data (grid, f_fwst     ) !waste heat flux from heat or cool AC [W/m2]
         call allocate_block_data (grid, f_fach     ) !flux from inner and outter air exchange [W/m2]
         call allocate_block_data (grid, f_fahe     ) !flux from metabolism and vehicle [W/m2]
         call allocate_block_data (grid, f_fhah     ) !sensible flux from heating [W/m2]
         call allocate_block_data (grid, f_fvehc    ) !flux from vehicle [W/m2]
         call allocate_block_data (grid, f_fmeta    ) !flux from metabolism [W/m2]
         call allocate_block_data (grid, f_senroof  ) !sensible heat flux from roof [W/m2]
         call allocate_block_data (grid, f_senwsun  ) !sensible heat flux from sunlit wall [W/m2]
         call allocate_block_data (grid, f_senwsha  ) !sensible heat flux from shaded wall [W/m2]
         call allocate_block_data (grid, f_sengimp  ) !sensible heat flux from impervious road [W/m2]
         call allocate_block_data (grid, f_sengper  ) !sensible heat flux from pervious road [W/m2]
         call allocate_block_data (grid, f_senurbl  ) !sensible heat flux from urban vegetation [W/m2]
         call allocate_block_data (grid, f_lfevproof) !latent heat flux from roof [W/m2]
         call allocate_block_data (grid, f_lfevpgimp) !latent heat flux from impervious road [W/m2]
         call allocate_block_data (grid, f_lfevpgper) !latent heat flux from pervious road [W/m2]
         call allocate_block_data (grid, f_lfevpurbl) !latent heat flux from urban vegetation [W/m2]
