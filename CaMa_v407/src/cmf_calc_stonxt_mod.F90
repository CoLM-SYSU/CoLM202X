...
USE YOS_CMF_INPUT,      ONLY: LGDWDLY,   DT,  LWEVAP, LWINFILT
USE YOS_CMF_MAP,        ONLY: NSEQALL,D2GRAREA
USE YOS_CMF_PROG,       ONLY: D2RIVOUT,  D2FLDOUT,  D2RIVSTO,  D2FLDSTO,  D2RUNOFF
USE YOS_CMF_PROG,       ONLY: D2GDWSTO,  D2GDWRTN,  D2ROFSUB,  D2WEVAP,D2WINFILT
USE YOS_CMF_DIAG,       ONLY: D2RIVINF,  D2FLDINF,  D2PTHOUT,  D2PTHINF,  D2FLDFRC, D2FLDARE,&
...
DO ISEQ=1, NSEQALL
  ...
  IF (LWEVAP) THEN
    ...
    D2FLDSTO(ISEQ,1) = MAX(D2FLDSTO(ISEQ,1) - DWEVAPEX,0.0D0) 
  ...
  IF (LWINFILT) THEN
    ...
    !! Assuming "potential water evaporation", multiplied by flood area fraction
    !! Limited by total amount of flooplain storage 
    D2WINFILTEX = MIN(D2FLDSTO(ISEQ,1),D2FLDFRC(ISEQ,1)*DT*D2WINFILT(ISEQ,1))
    D2FLDSTO(ISEQ,1) = D2FLDSTO(ISEQ,1) - DWINFILTEX 
